...
options : [
    ...
    action : async () => {
            if(ConfigManager.getKey('deepseek')){
                ...
                switch(response) {
                    case `Use with current model (${ColorText.green(obj.model)})`:
                        instance_config.deepseek_token = obj.token
                        instance_config.deepseek_model = obj.model
                        delete instance_config.openai_token
                        delete instance_config.openai_model
                        delete instance_config.deepinfra_token
                        delete instance_config.deepinfra_model
                ...
                if(await MenuCLI.ask('Save key and model?',{options : ['yes','no']}) == 'yes'){ConfigManager.setKey('deepseek',final_object)}
                instance_config.deepseek_token = final_object.token
                instance_config.deepseek_model = final_object.model
                delete instance_config.openai_token
                delete instance_config.openai_model
                delete instance_config.deepinfra_token
                delete instance_config.deepinfra_model
